...
def post(api_url, payload):
  print(colored("[POST]", 'white', 'on_yellow', attrs=['bold']) + " " + api_url + " ... ", end="")
...
def put(api_url, payload):
  print(colored("[PUT]", 'blue', 'on_blue', attrs=['bold']) + " " + api_url + " ... ", end="")
...
def format_word(word_data):
  ...
  plural_word_text = word_data["plural_nouns"][0]["target_text"] if (not word_data["no_plural"]) and word_data["plural_nouns"] else ''
...
def format_word_type(word_data):
  ...
  display_text = colored(wordtype_mappings.get(abbrev_text, abbrev_text), word_type_color)
...
def gather_parallel_word_details(word_id):
  ...
  params = [
    ...
    ['faztaa', ("https://api.faztaa.com/api/search/en/devi/" + word["target_text"])],
    ['examples', (f"https://seedlang.com/api/sentences?word_id={word_id}&sort=20&vocab_list=level_abbreviation,%20target_text&vocab_trainer=true")],
    ['is_added', (f"https://seedlang.com/api/cards?filters[user_id]={USER_ID}&filters[word_id]={word_id}&filters[sentence_id]=false&limit=20&page=1&my_reviews=true")]
  ]
...
def print_word_definition(word_id):
  ...
  rich_console.print(
    Panel(
      f"\n{ formatted_word }\n",                                                # word
      title=format_word_type(word) + " - " + word['word_type']['name'],         # word type
  ...
  print(colored("Listen the word in contexts: ", 'white', attrs=['bold']) + f"https://youglish.com/pronounce/{word['target_text']}/german")
  ...
  for translation in translations:
    if translation["rejected"]: continue
  ...
  if len(word_examples) != 0:
    ...
    for example in word_examples[:5]:
      ...
      for i in range(len(example_word_associations)):
        if example_word_associations[i]['word']['root_word']['id'] == word['id']:
  ...
  for translation in translations:
    ...
    if len(translation['examples']) == 0: continue

    render_lines += [ "\n\tExamples:" ]
    for example in translation['examples'][:5]:
      render_lines += [ ("\t- _" + example['e'] + "_").replace(
        word['target_text'],
        colored(word['target_text'], get_word_type_color(word['word_type']['abbreviation']), attrs=['bold'])
      ) ]
  ...
  if word['word_type']['abbreviation'] == 'vrb':
    ...
    verb_conjugation_table.add_row(
      "ich", *conjugation_mappings['singular_1p'],
    )
    verb_conjugation_table.add_row(
      "du", *conjugation_mappings['singular_2p'],
    )
    verb_conjugation_table.add_row(
      "ihr", *conjugation_mappings['plural_2p'],
    )
    verb_conjugation_table.add_row(
      "er/sie/es", *conjugation_mappings['singular_3p'],
    )
    verb_conjugation_table.add_row(
      "wir/sie/Sie/Sie", *conjugation_mappings['plural_1p'],
    )
  ...
  rich_console.print( Markdown(
    ...
    "- **DWDS**:       " + f"https://www.dwds.de/wb/{word['target_text']}" + "\n" + \
    "- **Wiktionary**: " + f"https://de.wiktionary.org/wiki/{word['target_text']}" + "\n" + \
    "- **Collins**:    " + f"https://www.collinsdictionary.com/dictionary/german-english/{word['target_text']}" + "\n" + \
    "- **Seedlang**:   " + f"https://seedlang.com/vocab/words/{word['id']}" + "\n" + \
    "- **Faztaa**:     " + f"https://faztaa.com/search/word/{word['target_text']}?hl=en" + "\n" + \
    "- **Cambridge**:  " + f"https://dictionary.cambridge.org/dictionary/german-english/{word['target_text']}" + "\n" + \
    "- **Google Img**: " + f"https://www.google.com/search?q={word['target_text']}&udm=2" + "\n"
  ) )

  # print helper command -------------------------------------------------------
  print()
  print("To quickly add the word to your deck, please use the following command:")
  print("\t" + colored("python .\seedlang.py add-review", 'white') + " " + colored(word_id, 'cyan'))
  print()
...
def add_to_review(word_id):
  ...
  print(colored(on_due_cards_stat['count'], 'red', attrs=['bold']) + " / " + str(total_count) + " cards")
  print()
...
def review():
  webbrowser.open("https://seedlang.com/reviews/decks/59c08cb3-bdb7-447b-b573-fb68056463d8")
